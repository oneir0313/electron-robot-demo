...
ipcMain.on('robot-error', (event, arg) => {
    if ('errorType' in arg) {
        ...
        if (arg.errorType === 'betFail') {
            console.error(`robot[${arg.account}] bet fail: ${arg.message}, code: ${arg.code}`);
        }
        return;
    }
    console.error(`robot[${arg.account}]: error: ${arg.error.message} stack: ${arg.error.stack}`);
});
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'test') {
            ...
            robot = new BrowserWindow({
                width: 500, height: 500, webPreferences: {
...
async function main() {
    ...
    router
        ...
        .post('/is-ready', ctx => {
            ...
            const totalAmount = robotAmountList.size > 0 ? [...robotAmountList.values()].reduce((arr, cur) => arr + cur) : 0;
            let isReady = false;
        ...
        .post('/robot/:number', async ctx => {
            try {
                ...
                const robotNames = excuteTimes == 1 ? [ctx.request.body.account] : [...Array(excuteTimes).keys()].map(el => ctx.request.body.account + String(el + 1).padStart(3, '0'));
                ctx.status = 202;
        ...
        .delete('/robot/:testNo', async ctx => {
            ...
            if (ctx.params.testNo) {
                // 如果輸入參數為all 將刪除所有
                const robots = ctx.params.testNo === 'all' ?
                    ...
                    if ('close' in robot) {
                        if (robotData[robot.name] !== 'NA') {
                            try {
                                ...
                                console.error(`delete robot: close robot error: ${error.message}`);
                            }
                        }
                    }
                });
